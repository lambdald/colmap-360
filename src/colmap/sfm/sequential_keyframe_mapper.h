...
//
//  SequentialKeyframeMapper mapper(&database_cache);
...
//
class SequentialKeyframeMapper {
 public:
  struct Options {
    ...
    enum class ImageSelectionMethod {
      ADJACENT_MAX_VISIBLE_POINTS_NUM,
      ADJACENT_MAX_VISIBLE_POINTS_RATIO,
      ADJACENT_MIN_UNCERTAINTY,
    };
    ImageSelectionMethod image_selection_method =
        ImageSelectionMethod::ADJACENT_MIN_UNCERTAINTY;
  ...
  explicit SequentialKeyframeMapper(
 ...
 private:
  ...
  std::unordered_map<image_t, size_t> image_orders_;
  size_t min_reg_image_order_ = 0;
  size_t max_reg_image_order_ = 0;
  std::unordered_set<image_t> keyframe_ids_;
